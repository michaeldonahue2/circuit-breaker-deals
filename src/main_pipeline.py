...
def ai_enrich(deals):
    ...
    for deal in deals:
        try:
            prompt = f"Analyze deal: '{deal['title']}'. JSON: headline (max 6 words), why_good (6 words), discount_guess, category (Tech, Home, Audio)."
...
"""
 ...
 <item>
  ...
  <description>{deal['why_good']}</description>
  <link>{deal['link']}</link>
  <guid>{deal['link']}</guid>
...
def generate_site(deals):
    ...
    <head>
        ...
        <style>
            ...
            .logo {{ font-size: 1.8rem; font-weight: 900; color: white; letter-spacing: -1px; }}
    ...
    <body>
        <div class="nav">
            <div class="logo">BETTER<span>AMAZON</span>PRICES</div>
        </div>
        <div class="hero">
